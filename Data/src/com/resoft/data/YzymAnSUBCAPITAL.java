...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于YZYM_AN_SUBCAPITAL执行同步更新
 * </p>
 * <p>
...
public class YzymAnSUBCAPITAL {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		System.out.println("执行YzymAnSUBCAPITAL");
		//获取sqlserver中S_EXT_TIMESTAMP中最大的时间
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.YZYM_AN_SUBCAPITAL ";
		...
		String sql = "SELECT t.*   from YZYM_AN_SUBCAPITAL t " +
				"where S_EXT_TIMESTAMP > to_date('"+maxDate+"','yyyy-mm-dd hh24:mi:ss')  ORDER BY t.S_EXT_TIMESTAMP";//获取数据列表
		try {
			pre = con.prepareStatement(sql);
			resultSet = pre.executeQuery();
			//字段名称数组
			String[] columu = {"INVID","ANCHEID","INVNAME","LISUBCONAM","SUBCONDATE","SUBCONFORM","SUBCURRENCY","LIACCONAM",
					"ACCCONDATE","ACCCONFORM","ACCCURRENCY","S_EXT_TIMESTAMP"   };
			int i=0;
			while (resultSet.next()) {
				List<Object> minList = new ArrayList<Object>();
				for(String each:columu){
					if( each == "S_EXT_TIMESTAMP"||each == "ACCCONDATE"||each == "SUBCONDATE"){
						minList.add(resultSet.getTimestamp(each));
					}else if(each == "LISUBCONAM" ||each == "LIACCONAM"   ) {
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.YZYM_AN_SUBCAPITAL values (?,?,?,?,?,?,?,?,?,?,?,?)");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select INVID from usci.YZYM_AN_SUBCAPITAL where INVID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.YZYM_AN_SUBCAPITAL set INVID = ? ,ANCHEID = ? ,INVNAME = ? ,LISUBCONAM = ? ," +
		    				"SUBCONDATE = ? ,SUBCONFORM = ? ,SUBCURRENCY = ? ,LIACCONAM = ? ,ACCCONDATE = ? ,ACCCONFORM = ? ," +
		    				"ACCCURRENCY = ? ,S_EXT_TIMESTAMP = ?    where INVID = '"+etpsid+"'");
	    	if(resultSet.next()){
