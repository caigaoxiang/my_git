...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于YZYM_E_FINANCE执行同步更新
 * </p>
 * <p>
...
public class YzymEFinance {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		System.out.println("执行YzymEFinance");
		//获取sqlserver中S_EXT_TIMESTAMP中最大的时间
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.YZYM_E_FINANCE ";
		...
		String sql = "SELECT t.*  from YZYM_E_FINANCE t " +
				"where S_EXT_TIMESTAMP > to_date('"+maxDate+"','yyyy-mm-dd hh24:mi:ss')  ORDER BY t.S_EXT_TIMESTAMP";//获取数据列表
		try {
			pre = con.prepareStatement(sql);
			resultSet = pre.executeQuery();
			//字段名称数组
			String[] columu = {"ETPSID","NAME","CERTYPE","CERNO","TEL","MOBTEL","EMAIL","S_EXT_TIMESTAMP"  };
			int i=0;
			while (resultSet.next()) {
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.YZYM_E_FINANCE values (?,?,?,?,?,?,?,? )");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select ETPSID from usci.YZYM_E_FINANCE where ETPSID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.YZYM_E_FINANCE set ETPSID = ?,  NAMEE = ? ,CERTYPE = ? ,CERNO = ? ,TEL = ? ,MOBTEL= ? , " +
		    				" EMAIL = ? ,S_EXT_TIMESTAMP= ?    where ETPSID = '"+etpsid+"'");
	    	if(resultSet.next()){
