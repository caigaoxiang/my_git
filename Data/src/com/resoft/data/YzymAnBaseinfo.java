...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于YZYM_AN_BASEINFO执行同步更新
 * </p>
 * <p>
		...
		try {
			...
			while (resultSet.next()) {
				...
				for(String each:columu){
					if( each == "S_EXT_TIMESTAMP"||each == "ANCHEDATE"    ){
						minList.add(resultSet.getTimestamp(each));
					}else if(each == "EMPNUM" ||each == "WOMEMPNUM"|| each == "ASSGRO" ||each == "LIAGRO"||each == "VENDINC" ||each == "MAIBUSINC"||
							each == "PROGRO" ||each == "NETINC"||each == "RATGRO"||each == "TOTEQU" ) {
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.YZYM_AN_BASEINFO values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select ANCHEID from usci.YZYM_AN_BASEINFO where ANCHEID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.YZYM_AN_BASEINFO set ANCHEID = ? ,ANCHEDATE = ? ,ANCHEYEAR = ? ,REGNO = ? ," +
		    				"UNISCID = ? ,ENTNAME = ? ,ENTTYPE = ? ,TEL = ? ,ADDR = ? ,POSTALCODE = ? ,EMAIL = ? ,BUSST = ? ," +
		    				"HOLDINGSMSG = ? ,MAINBUSIACT = ? ,EMPNUM = ? ,WOMEMPNUM = ? ,ASSGRO = ? ,LIAGRO = ? ,VENDINC = ? ," +
		    				"MAIBUSINC = ? ,PROGRO = ? ,NETINC = ? ,RATGRO = ? ,TOTEQU = ? ,HASWEBSITE = ? ,HASTRANSAM = ? ," +
		    				"S_EXT_TIMESTAMP = ? ,ENT_UUID = ?    where ANCHEID = '"+etpsid+"'");
	    	if(resultSet.next()){
