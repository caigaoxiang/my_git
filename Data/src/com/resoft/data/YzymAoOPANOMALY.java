...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于YZYM_AO_OPANOMALY执行同步更新
 * </p>
 * <p>
...
public class YzymAoOPANOMALY {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		System.out.println("执行YzymAoOPANOMALY");
		//获取sqlserver中S_EXT_TIMESTAMP中最大的时间
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.YZYM_AO_OPANOMALY ";
		...
		String sql = "SELECT t.*  from YZYM_AO_OPANOMALY t " +
				"where S_EXT_TIMESTAMP > to_date('"+maxDate+"','yyyy-mm-dd hh24:mi:ss')  ORDER BY t.S_EXT_TIMESTAMP";//获取数据列表
		try {
			pre = con.prepareStatement(sql);
			resultSet = pre.executeQuery();
			//字段名称数组
			String[] columu = {"EXCEPTID","UNISCID","ENTNAME","REGNO","SPECAUSE","ABNTIME","DECORG","REMEXCPRES",
					"REDECORG","REMDATE","S_EXT_TIMESTAMP"	};
			int i=0;
			while (resultSet.next()) {
				List<Object> minList = new ArrayList<Object>();
				for(String each:columu){
					if( each == "S_EXT_TIMESTAMP"||each == "ABNTIME"||each == "REMDATE"  ){
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.YZYM_AO_OPANOMALY values (?,?,?,?,?,?,?,?,?,?,? )");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select EXCEPTID from usci.YZYM_AO_OPANOMALY where EXCEPTID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.YZYM_AO_OPANOMALY set EXCEPTID =? ,UNISCID=? ,ENTNAME=? ,REGNO=? ,SPECAUSE=? ," +
		    				"ABNTIME=? ,DECORG=? ,REMEXCPRES=? ,REDECORG=? ,REMDATE=? ,S_EXT_TIMESTAMP=?  where EXCEPTID = '"+etpsid+"'");
	    	if(resultSet.next()){
