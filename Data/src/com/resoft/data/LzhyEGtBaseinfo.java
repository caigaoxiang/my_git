...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于LZHY_E_GT_BASEINFO执行同步更新
 * </p>
 * <p>
...
public class LzhyEGtBaseinfo {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from USCI.USCI.LZHY_E_GT_BASEINFO ";
		pre = conSqlserver.prepareStatement(sqlServer);
		resultSet = pre.executeQuery();
		System.out.println("执行LzhyEGtBaseinfo");
		String maxDate = null;
		while(resultSet.next()){
			maxDate =  resultSet.getDate(1)+" "+resultSet.getTime(1) ;
		}
		String sql = "SELECT t.* ,0 as dflag from LZHY_E_GT_BASEINFO t " +
				"where S_EXT_TIMESTAMP > to_date('"+maxDate+"','yyyy-mm-dd hh24:mi:ss')  ORDER BY S_EXT_TIMESTAMP";//获取数据列表
		try {
			...
			while (resultSet.next()) {
				...
				for(String each:columu){
					if( each == "ESTDATE"||each == "APPRDATE" ||each == "S_EXT_TIMESTAMP"  ){
						minList.add(resultSet.getTimestamp(each));
					}else if(each == "FUNDAM" ||each == "EMPNUM"   ) {
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into USCI.USCI.LZHY_E_GT_BASEINFO values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? )");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select PEID from USCI.USCI.LZHY_E_GT_BASEINFO where PEID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update USCI.USCI.LZHY_E_GT_BASEINFO set PEID = ?,INFOACTIONTYPE = ?,TRANAME = ?,UNISCID = ?," +
		    				"REGNO = ?,COMPFORM = ?,FUNDAM = ?,OPSCOPE = ?,ESTDATE = ?,APPRDATE = ?,NOTNO = ?,REGSTATE = ?," +
		    				"REGORG = ?,AREAORG = ?,ENTTYPE = ?,EMPNUM = ?,INDUSTRYCO = ?,OPLOC = ?,OPLOCDISTRICT = ?,PROLOC = ?," +
		    				"YIEDISTRICT = ?,POSTALCODE = ?,TEL = ?,S_EXT_TIMESTAMP = ?,dflag = ?,XGJL=? where PEID = '"+etpsid+"'");
	    	if(resultSet.next()){
