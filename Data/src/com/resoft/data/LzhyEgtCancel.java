...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于LZHY_E_GT_CANCEL执行同步更新
 * </p>
 * <p>
...
public class LzhyEgtCancel {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.LZHY_E_GT_CANCEL ";
		pre = conSqlserver.prepareStatement(sqlServer);
		resultSet = pre.executeQuery();
		String maxDate = null;
		System.out.println("执行LzhyEgtCancel");
		while(resultSet.next()){
			maxDate =  resultSet.getDate(1)+" "+resultSet.getTime(1) ;
		}
		String sql = "SELECT t.*   from LZHY_E_GT_CANCEL t " +
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.LZHY_E_GT_CANCEL values (?,?,?,?,?)");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select PEID from usci.LZHY_E_GT_CANCEL where PEID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.LZHY_E_GT_CANCEL set PEID =? ,INFOACTIONTYPE =? ,CANDATE =? ,CANREGREA =? ," +
		    				"S_EXT_TIMESTAMP =?  where PEID = '"+etpsid+"'");
	    	if(resultSet.next()){
