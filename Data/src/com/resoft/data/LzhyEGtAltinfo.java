...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于LZHY_E_GT_ALTINFO执行同步更新
 * </p>
 * <p>
...
public class LzhyEGtAltinfo {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.LZHY_E_GT_ALTINFO ";
		pre = conSqlserver.prepareStatement(sqlServer);
		resultSet = pre.executeQuery();
		String maxDate = null;
		System.out.println("执行LzhyEGtAltinfo表");
		while(resultSet.next()){
			maxDate =  resultSet.getDate(1)+" "+resultSet.getTime(1) ;
		}
		String sql = "SELECT t.*  from LZHY_E_GT_ALTINFO t " +
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.LZHY_E_GT_ALTINFO values (?,?,?,?,?,?,?,? )");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select ALTID from usci.LZHY_E_GT_ALTINFO where ALTID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.LZHY_E_GT_ALTINFO set ALTID = ? ,PEID = ? ,NOTNO = ? ,ALTITEM = ? ,ALTBE = ? ," +
		    				"ALTAF = ? ,ALTDATE = ? ,S_EXT_TIMESTAMP = ?  where ALTID = '"+etpsid+"'");
	    	if(resultSet.next()){
