...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于YZYM_E_CONTACT执行同步更新
 * </p>
 * <p>
...
public class YzymEContact {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		System.out.println("执行YzymEContact");
		//获取sqlserver中S_EXT_TIMESTAMP中最大的时间
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.YZYM_E_CONTACT ";
		...
		String sql = "SELECT t.*  from YZYM_E_CONTACT t " +
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.YZYM_E_CONTACT values (?,?,?,?,?,?,?,? )");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select ETPSID from usci.YZYM_E_CONTACT where ETPSID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.YZYM_E_CONTACT set ETPSID = ?,  NAMEE = ? ,CERTYPE = ? ,CERNO = ? ,TEL = ? ,MOBTEL= ? , " +
