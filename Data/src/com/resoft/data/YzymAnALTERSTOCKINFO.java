...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于YZYM_AN_ALTERSTOCKINFO执行同步更新
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class YzymAnALTERSTOCKINFO {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.YZYM_AN_ALTERSTOCKINFO ";
		pre = conSqlserver.prepareStatement(sqlServer);
		resultSet = pre.executeQuery();
		System.out.println("执行YzymAnALTERSTOCKINFO");
		String maxDate = null;
		while(resultSet.next()){
			maxDate =  resultSet.getDate(1)+" "+resultSet.getTime(1) ;
		}
		String sql = "SELECT t.*   from YZYM_AN_ALTERSTOCKINFO t " +
				"where S_EXT_TIMESTAMP > to_date('"+maxDate+"','yyyy-mm-dd hh24:mi:ss') ORDER BY t.S_EXT_TIMESTAMP ";//获取数据列表
		try {
			pre = con.prepareStatement(sql);
			resultSet = pre.executeQuery();
			//字段名称数组
			String[] columu = {"INVALTID","ANCHEID","INV","TRANSAMPR","TRANSAMAFT","ALTDATE","S_EXT_TIMESTAMP"   };
			int i=0;
			while (resultSet.next()) {
				List<Object> minList = new ArrayList<Object>();
				for(String each:columu){
					if( each == "S_EXT_TIMESTAMP"||each == "ALTDATE"    ){
						minList.add(resultSet.getTimestamp(each));
					}else if(each == "TRANSAMPR" ||each == "TRANSAMAFT"   ) {
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.YZYM_AN_ALTERSTOCKINFO values (?,?,?,?,?,?,?)");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select INVALTID from usci.YZYM_AN_ALTERSTOCKINFO where INVALTID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.YZYM_AN_ALTERSTOCKINFO set INVALTID =? ,ANCHEID =? ,INV =? ,TRANSAMPR =? ," +
		    				"TRANSAMAFT =? ,ALTDATE =? ,S_EXT_TIMESTAMP =?    where INVALTID = '"+etpsid+"'");
	    	if(resultSet.next()){
