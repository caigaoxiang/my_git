...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于YZYM_AN_WEBSITEINFO执行同步更新
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class YzymAnWEBSITEINFO {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		System.out.println("执行YzymAnWEBSITEINFO");
		//获取sqlserver中S_EXT_TIMESTAMP中最大的时间
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.YZYM_AN_WEBSITEINFO ";
		...
		String sql = "SELECT t.*   from YZYM_AN_WEBSITEINFO t " +
				"where S_EXT_TIMESTAMP > to_date('"+maxDate+"','yyyy-mm-dd hh24:mi:ss') ORDER BY t.S_EXT_TIMESTAMP ";//获取数据列表
		try {
			pre = con.prepareStatement(sql);
			resultSet = pre.executeQuery();
			//字段名称数组
			String[] columu = {"WEBID","ANCHEID","WEBTYPE","WEBSITNAME","WEBSITE","S_EXT_TIMESTAMP"   };
			int i=0;
			while (resultSet.next()) {
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.YZYM_AN_WEBSITEINFO values (?,?,?,?,?,?)");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select WEBID from usci.YZYM_AN_WEBSITEINFO where WEBID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.YZYM_AN_WEBSITEINFO set WEBID = ? ,ANCHEID = ? ,WEBTYPE = ? ,WEBSITNAME = ? ," +
		    				"WEBSITE = ? ,S_EXT_TIMESTAMP = ?     where WEBID = '"+etpsid+"'");
	    	if(resultSet.next()){
