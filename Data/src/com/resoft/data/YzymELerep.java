...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于YZYM_E_LEREP执行同步更新
 * </p>
 * <p>
...
public class YzymELerep {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		System.out.println("执行YzymELerep");
		//获取sqlserver中S_EXT_TIMESTAMP中最大的时间
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.YZYM_E_LEREP ";
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.YZYM_E_LEREP values (?,?,?,?,?,?,?,?,?,?,?,? )");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select MEMBERID from usci.YZYM_E_LEREP where MEMBERID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.YZYM_E_LEREP set  MEMBERID =? ,ETPSID =? ,NAME =? ,COUNTRY =? ,CERTYPE =? ," +
		    				"CERNO =? ,TEL =? ,MOBTEL =? ,EMAIL =? ,S_EXT_TIMESTAMP =? ,BLICTYPE =? ,BLICNO =? where MEMBERID = '"+etpsid+"'");
	    	if(resultSet.next()){
