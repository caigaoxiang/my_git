...
/**
 * <p>
 * YzymeAlt類主要用於-.主要用于LZHY_E_GT_MEMBER执行同步更新
 * </p>
 * <p>
...
public class LzhyEgtMember {
	...
	public static List<List<Object>> tableInput() throws FileNotFoundException,SQLException {
		...
		String sqlServer = "SELECT MAX(S_EXT_TIMESTAMP) from usci.LZHY_E_GT_MEMBER ";
		pre = conSqlserver.prepareStatement(sqlServer);
		resultSet = pre.executeQuery();
		System.out.println("执行LzhyEgtMember");
		String maxDate = null;
		while(resultSet.next()){
			maxDate =  resultSet.getDate(1)+" "+resultSet.getTime(1) ;
		}
		String sql = "SELECT t.*   from LZHY_E_GT_MEMBER t " +
				"where S_EXT_TIMESTAMP > to_date('"+maxDate+"','yyyy-mm-dd hh24:mi:ss') ORDER BY t.S_EXT_TIMESTAMP ";//获取数据列表
		try {
			pre = con.prepareStatement(sql);
			resultSet = pre.executeQuery();
			//字段名称数组
			String[] columu = {"MEMID","PEID","NAME","CERTYPE","CERNO","S_EXT_TIMESTAMP" };
			int i=0;
			while (resultSet.next()) {
...
//	    Statement stat = null;
	    PreparedStatement pstInsert = con
	            .prepareStatement("insert into usci.LZHY_E_GT_MEMBER values (?,?,?,?,?,?)");
	    PreparedStatement pstUpdate = null ;
	    int updateNUm = 0;
	    int insertNum = 0;
	    for (List<Object> minList: FindList) {
	    	String etpsid = minList.get(0).toString() ;
	    	String sql1 = "select MEMID from usci.LZHY_E_GT_MEMBER where MEMID = '"+etpsid+"'";
	    	pre = con.prepareStatement(sql1);
	    	resultSet = pre.executeQuery();
	    	 pstUpdate = con
		    		.prepareStatement("update usci.LZHY_E_GT_MEMBER set MEMID =? ,PEID =? ,NAMEE =? ,CERTYPE =? ," +
		    				"CERNO = ? , S_EXT_TIMESTAMP =?  where MEMID = '"+etpsid+"'");
	    	if(resultSet.next()){
